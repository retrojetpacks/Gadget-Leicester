...
int force_treebuild_single(int npart, struct unbind_data *mp)
{
  ...
  morton_list = (peanokey *) mymalloc(NumPart * sizeof(peanokey));
...
void force_exchange_pseudodata(void)
{
  ...
  DomainMoment = (struct DomainNODE *) mymalloc(NTopleaves * sizeof(struct DomainNODE));
...
void force_finish_kick_nodes(void)
{
  ...
  counts = (int *) mymalloc(sizeof(int) * NTask);
  counts_dp = (int *) mymalloc(sizeof(int) * NTask);
  offset_list = (int *) mymalloc(sizeof(int) * NTask);
  offset_dp = (int *) mymalloc(sizeof(int) * NTask);
  offset_vmax = (int *) mymalloc(sizeof(int) * NTask);

  domainDp_loc = (MyLongDouble *) mymalloc(DomainNumChanged * 3 * sizeof(MyLongDouble));
#ifdef SCALARFIELD
  domainDp_dm_loc = (MyLongDouble *) mymalloc(DomainNumChanged * 3 * sizeof(MyLongDouble));
#endif
  domainVmax_loc = (MyFloat *) mymalloc(DomainNumChanged * sizeof(MyFloat));
  ...
  domainDp_all = (MyLongDouble *) mymalloc(totDomainNumChanged * 3 * sizeof(MyLongDouble));
#ifdef SCALARFIELD
  domainDp_dm_all = (MyLongDouble *) mymalloc(totDomainNumChanged * 3 * sizeof(MyLongDouble));
#endif
  domainVmax_all = (MyFloat *) mymalloc(totDomainNumChanged * sizeof(MyFloat));

  domainList_all = (int *) mymalloc(totDomainNumChanged * sizeof(int));
...
void force_update_hmax(void)
{
  ...
  counts = (int *) mymalloc(sizeof(int) * NTask);
  offset_list = (int *) mymalloc(sizeof(int) * NTask);
  offset_hmax = (int *) mymalloc(sizeof(int) * NTask);

  domainHmax_loc = (MyFloat *) mymalloc(DomainNumChanged * 2 * sizeof(MyFloat));
  ...
  domainHmax_all = (MyFloat *) mymalloc(totDomainNumChanged * 2 * sizeof(MyFloat));
  domainList_all = (int *) mymalloc(totDomainNumChanged * sizeof(int));
 ...
 */
int force_treeevaluate(int target, int mode, int *nexport, int *nsend_local)
{
  struct NODE *nop = 0;
  int no, nodesinlist, ninteractions, ptype, task, nexport_save, listindex = 0;
			  ...
			  if(*nexport >= All.BunchSize)
			    {
			      ...
			      *nexport = nexport_save;
			      if(nexport_save == 0)
				endrun(12998);	/* in this case, the buffer is too small to process even a single particle */
	  ...
	  if(r >= h)
		...
		wp =
	      ...
	      /*second derivates needed -> calculate them from softend potential,
	         (see Gadget 1 paper and there g2 function) */
	      if(u < 0.5)
		fac2 = mass * h5_inv * (-76.8 + 96 * u);
	      else
		fac2 = mass * h5_inv * (0.2 / (u * u * u * u * u) - 48 / u + 76.8 - 32 * u);
#endif
	    }
#ifdef EVALPOTENTIAL
#ifdef PERIODIC
...
#if defined(DISTORTIONTENSOR) || defined(OUTPUT_TIDALTENSOR)
	  /*
	     tidal_tensor[] = Matrix of second derivatives of grav. potential, symmetric:
	     |Dxx Dxy Dxz|   |tidal_tensor[0] tidal_tensor[1] tidal_tensor[2]|
	     |Dyx Dyy Dyz| = |                tidal_tensor[3] tidal_tensor[4]| 
	     |Dzx Dzy Dzz|   |                                tidal_tensor[5]|
	   */
	  tidal_tensor[0] += FLT(fac + dx * dx * fac2);
	  tidal_tensor[1] += FLT(dx * dy * fac2);
	  tidal_tensor[2] += FLT(dx * dz * fac2);
	  tidal_tensor[3] += FLT(fac + dy * dy * fac2);
	  tidal_tensor[4] += FLT(dy * dz * fac2);
	  tidal_tensor[5] += FLT(fac + dz * dz * fac2);
#endif
	  if(mass > 0)
	    ninteractions++;
...
#endif
#if defined(DISTORTIONTENSOR) || defined(OUTPUT_TIDALTENSOR)
      P[target].tite.dtidal_tensor[0] = tidal_tensor[0];
      P[target].tite.dtidal_tensor[1] = tidal_tensor[1];
      P[target].tite.dtidal_tensor[2] = tidal_tensor[2];
      P[target].tite.dtidal_tensor[3] = tidal_tensor[3];
      P[target].tite.dtidal_tensor[4] = tidal_tensor[4];
      P[target].tite.dtidal_tensor[5] = tidal_tensor[5];
#endif
    }
  else
    {
...
#if defined(DISTORTIONTENSOR) || defined(OUTPUT_TIDALTENSOR)
      GravDataResult[target].tidal_tensor[0] = tidal_tensor[0];
      GravDataResult[target].tidal_tensor[1] = tidal_tensor[1];
      GravDataResult[target].tidal_tensor[2] = tidal_tensor[2];
      GravDataResult[target].tidal_tensor[3] = tidal_tensor[3];
      GravDataResult[target].tidal_tensor[4] = tidal_tensor[4];
      GravDataResult[target].tidal_tensor[5] = tidal_tensor[5];
#endif
 ...
 *  table.
 */
int force_treeevaluate_shortrange(int target, int mode, int *nexport, int *nsend_local)
{
  struct NODE *nop = 0;
  int no, nodesinlist, ptype, ninteractions, tabindex, task, nexport_save, listindex = 0;
  double r2, dx, dy, dz, mass, r, fac, u, h, h_inv, h3_inv;
  double pos_x, pos_y, pos_z, aold;
  double eff_dist;
			  ...
			  if(*nexport >= All.BunchSize)
			    {
			      *nexport = nexport_save;
			      if(nexport_save == 0)
				endrun(12999);	/* in this case, the buffer is too small to process even a single particle */
			      for(task = 0; task < NTask; task++)
				nsend_local[task] = 0;
			      for(no = 0; no < nexport_save; no++)
				nsend_local[DataIndexTable[no].Task]++;
 ...
 */
int force_treeevaluate_ewald_correction(int target, int mode, int *nexport, int *nsend_local)
{
  struct NODE *nop = 0;
  int no, cost, nexport_save, listindex = 0;
			  ...
			  if(*nexport >= All.BunchSize)
			    {
			      *nexport = nexport_save;
			      if(nexport_save == 0)
				endrun(13000);	/* in this case, the buffer is too small to process even a single particle */
			      for(task = 0; task < NTask; task++)
				nsend_local[task] = 0;
			      for(no = 0; no < nexport_save; no++)
				nsend_local[DataIndexTable[no].Task]++;
...
void force_treeallocate(int maxnodes, int maxpart)
{
  ...
  DomainNodeIndex = (int *) mymalloc(bytes = NTopleaves * sizeof(int));
  ...
  if(!(Nextnode = (int *) mymalloc(bytes = (maxpart + NTopnodes) * sizeof(int))))
    {
