...
/* SHC END */
  ...
  for(i = 0; i < NumPart; i++)
    {
      ...
      /* SHC BEGIN */
        ...
        if (P[i].Type == 3 && P[i].Mass != 0.)
          sys.EnergyRadComp += P[i].OldPhotonMomentum *C/All.UnitVelocity_in_cm_per_s;
#ifdef CHECK_ENERGY_CONSERVATION
        if (P[i].Type == 5)
          sys.EnergySinkInt += (1.99e31/All.UnitMass_in_g)/(1.e11/All.UnitLength_in_cm) * P[i].Mass;
#endif
#endif
      /* SHC END */
...
#ifdef RAD_ACCEL
	    vel[j] += SphP[i].ra.RadAccel[j] * dt_hydrokick;
#endif
#ifdef DUST
	    //	    vel[j] += SphP[i].da.DragAccel[j] * dt_hydrokick;
#endif
#ifdef GAS_FIXED
	    vel[j] = 0.;
#endif
  ...
  /* SHC BEGIN */
#ifdef CHECK_ENERGY_CONSERVATION
  MPI_Reduce(&sys.EnergyRadComp, &SysState.EnergyRadComp, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
  MPI_Reduce(&sys.EnergySinkInt, &SysState.EnergySinkInt, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
#endif
  /* SHC END */
...
#ifdef DUST
  /*  printf(" total MOMENTUM of the system = (%g, %g, %g) and modulus %g \n", SysState.Momentum[0], 
