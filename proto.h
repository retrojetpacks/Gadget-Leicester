...
/* SHC */
int checksink_evaluate(int target, int mode, int *nexport, int *nsend_local, int *flag);
void checksink(int i, int *flag, int *nexport, int *nsend_local);
/* SHC */
...
int ngb_treefind_dust_active(MyDouble searchcenter[3], MyFloat hsml, int target, int *startnode, int mode,
				int *nexport, int *nsend_local, int ptype_want);
...
//int virtual_evaluate_density(int target, int mode, int *nexport, int *nsend_local);
int virtual_evaluate_select(int target, int mode, int *nexport, int *nsend_local);
int virtual_interact(int target, int mode, int *nexport, int *nsend_local);
...
int virtual_evaluate(int target, int mode, int *nexport, int *nsend_local);
int virtual_evaluate_swallow(int target, int mode, int *nexport, int *nsend_local);
int  virtual_compare_key(const void *a, const void *b);

#ifdef REAL_EOS 
double ene_per_gram(int idx, double rho, double temp); /* finds mean mol weight and spec internal energy for SPH particles */
...
double get_starformation_rate_fb(int i);
void gravity_tree(void);
void hydro_force(void);
void init(void);
#ifndef LT_STELLAREVOLUTION
void init_clouds(void);
void init_clouds_fb(void);
void integrate_sfr(void);
void integrate_sfr_fb(void);
#else
...
void rearrange_particle_sequence_fb(void);
