...
void pm_init_nonperiodic(void)
{
...
#if !defined(PERIODIC)
  if(!(kernel[0] = (fftw_real *) mymalloc(bytes = fftsize * sizeof(fftw_real))))
    {
...
#ifdef SCALARFIELD
  if(!(kernel_scalarfield[0] = (fftw_real *) mymalloc(bytes = fftsize * sizeof(fftw_real))))
    {
  ...
  if(!(kernel[1] = (fftw_real *) mymalloc(bytes = fftsize * sizeof(fftw_real))))
    {
...
#ifdef SCALARFIELD
  if(!(kernel_scalarfield[1] = (fftw_real *) mymalloc(bytes = fftsize * sizeof(fftw_real))))
    {
      ...
      if(recvTask < NTask)
	{
	  if(level > 0)
		 ...
		 || localfield_togo[recvTask * NTask + sendTask] > 0)
		{
		  ...
		  MPI_Sendrecv(localfield_globalindex + localfield_offset[recvTask],
			       localfield_togo[sendTask * NTask + recvTask] * sizeof(int), MPI_BYTE,
			       recvTask, TAG_NONPERIOD_B,
			       import_globalindex,
			       localfield_togo[recvTask * NTask + sendTask] * sizeof(int), MPI_BYTE,
