...
void GetMachNumber(struct sph_particle_data *Particle)
{
  ...
  rhs = fac_hsml * Particle->Hsml * atime / (csnd * Particle->Entropy) * Particle->e.DtEntropy * hubble_a;
...
int shock_conditions_fnd(const gsl_vector * v, void *auxParticle, gsl_vector * f)
{
  ...
  if(All.ComovingIntegrationOn)
    {
      /* CR definitions: */
      rho1 = Particle->a2.Density / mycube(All.Time);
      PCR1 = CR_Particle_Pressure(Particle) * pow(All.Time, -3.0 * GAMMA);
      eCR1 = CR_Particle_SpecificEnergy(Particle) * rho1;
      gCR = Particle->CR_Gamma0;
    ...
    {
      /* CR definitions: */
      rho1 = Particle->a2.Density;
      PCR1 = CR_Particle_Pressure(Particle);
      eCR1 = CR_Particle_SpecificEnergy(Particle) * rho1;
      gCR = Particle->CR_Gamma0;
  ...
  if(x < 0.0)
    {
      ...
      printf("  rho1 = %e;  hsml     = %e;  Ath1 = %e;  dAth1dt = %e;\n",
	     Particle->a2.Density, Particle->Hsml, Particle->Entropy, Particle->e.DtEntropy);
      printf("  P1   = %e;  PCR1     = %e;  eCR1 = %e;  gCR     = %e;\n\n",
	     Particle->Pressure, PCR1 * pow(atime, 3.0 * GAMMA), eCR1 / rho1, Particle->CR_Gamma0);
      fflush(stdout);
    }
  assert(x >= 0.0);
...
int shock_conditions_df(const gsl_vector * v, void *auxParticle, gsl_matrix * J)
{
  ...
  if(All.ComovingIntegrationOn)
  ...
  else
    {
      ...
      rho1 = Particle->a2.Density;
      PCR1 = CR_Particle_Pressure(Particle);
      eCR1 = CR_Particle_SpecificEnergy(Particle) * Particle->a2.Density;;
      gCR = Particle->CR_Gamma0;
  ...
  df00 =
    ...
    1. / P2 * (2. * Pth1 * rho1g1 * rho2g2 * rho2g2 * (P2 - P1) *
	       (P2 / rho1g1 - P1 / rho2g2) *
  ...
  df01 =
    ...
    1. / P2 * (8. * Pth1 * rho1g1 * x * rho2g2 * rho2g2 * (P2 - P1) *
	       (P2 / rho1g1 - P1 / rho2g2) *
	       (Pth1 * (x + 4. * z) + PCR1 * xgCR * (1 + (gCR - gth) * log(rho1 * x))));
...
void GetMachNumberCR(struct sph_particle_data *Particle)
{
  ...
  static double x0, y0, z0, t0, x, y, z, xiter;
  ...
  if(SwitchDiagnostic == 1)
    {
      ...
      printf("  P1   = %e;  PCR1     = %e;  eCR1   = %e;  gCR     = %e;\n\n",
	     ...
	     CR_Particle_SpecificEnergy(Particle) * Particle->a2.Density, Particle->CR_Gamma0);
      fflush(stdout);
    }
  ...
  if(Mestimate < 1.4 || XCR < 0.01)
      ...
      if((XCR >= 0.01) && (XCR < XCRSwitch))
	{
	  ...
	  while((status == GSL_CONTINUE) && (iter < 50) && (xiter > 0.));
