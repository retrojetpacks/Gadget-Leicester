...
static int numPart;		/* this is equal to NumPart for a normal
				   domain decomposition. In case we do SUBFIND, this can
				   be smaller, and equal to the local particles in the
				   respective group */
...
void domain_Decomposition(void)
{
  ...
  if(All.NumForcesSinceLastDomainDecomp >= All.TotNumPart * All.TreeDomainUpdateFrequency
...
#if defined(SFR) || defined(BLACK_HOLES) || defined(DUST)
      ...
      if(ThisTask == 0)
	{
      ...
      if(GrNr >= 0)
	{
	  numPart = NumPartGroup;

	  for(i = 0; i < NumPartGroup; i++)
	    if(P[i].GrNr != GrNr)
	      endrun(1222);

	  for(i = NumPartGroup; i < NumPart; i++)
	    if(P[i].GrNr == GrNr)
	      endrun(1123);	/* they are not all at the beginning */
	}
#endif
...
int domain_determineTopTree(void)
{
  ...
  for(i = 0; i < numPart; i++)
    {
      mp[i].key = Key[i] = peano_hilbert_key((int) ((P[i].Pos[0] - DomainCorner[0]) * DomainFac),
...
#ifdef MYSORT
  mysort_domain(mp, numPart, sizeof(struct peano_hilbert_data), domain_compare_key);
#else
  ...
  if(NTask == (1 << PTask))	/* the following algoritm only works for power of 2 */
    ...
    {
      domain_recursively_combine_topTree(0, NTask);
    }
