...
{
  return (Particle->a2.Density * HYDROGEN_MASSFRAC * IonizationGrade(Particle) / m_p);
}
...
{
  return (Particle->a2.Density * (3.0 * HYDROGEN_MASSFRAC + 1.0) / (4.0 * m_p));
}
...
static inline double Physical_Density(SphParticle * Particle)
{
  if(All.ComovingIntegrationOn)
    {
      return Particle->a2.Density / cube(All.Time);
    }
  else
    {
      return Particle->a2.Density;
