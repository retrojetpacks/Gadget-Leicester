...
static void serial_sort(void *base, size_t nmemb, size_t size, int (*compar) (const void *, const void *));
static void msort_serial_with_tmp(void *base, size_t n, size_t s, int (*compar) (const void *, const void *),
				  void *t);
static void parallel_merge(int master, int ncpu, size_t * nlist, size_t nmax, void *base, size_t nmemb,
...
void parallel_sort(void *base, size_t nmemb, size_t size, int (*compar) (const void *, const void *))
{
  ...
  serial_sort(base, nmemb, size, compar);

  nlist = mymalloc(NTask * sizeof(size_t));
  ...
  for(ncpu_in_group = 2; ncpu_in_group <= (1 << PTask); ncpu_in_group *= 2)
    {
      ...
      parallel_merge(master, ncpu_in_group, nlist, nmax, base, nmemb, size, compar);
    }
		    ...
		    void *base, size_t nmemb, size_t size, int (*compar) (const void *, const void *))
{
  ...
  if(ThisTask != master)
    ...
    {
      list_a = mymalloc(nmax * size);
      list_b = mymalloc(nmax * size);
      list_r = mymalloc(nmax * size);
...
static void serial_sort(void *base, size_t nmemb, size_t size, int (*compar) (const void *, const void *))
{
  ...
  void *tmp = (void *) mymalloc(storage);
