	...
	if(P[i].Type == 1)
	  {
	    ...
	    a = All.OmegaBaryon / All.Omega0 * d;
	    b = (All.Omega0 - All.OmegaBaryon) / All.Omega0 * d;
...
void empty_read_buffer(enum iofields blocknr, int offset, int pc, int type)
{
  ...
  switch (blocknr)
	...
	/* { */
	/*   //	  if (P[offset + n].Type == 2  &&  RestartFlag == 0) */
	/*   if (P[offset + n].Type == 100) */
	/*     { */
	/*       P[offset + n].Mass = (*fp++) * All.InitialDustMass; */
	/*     } */
	/*   else */
	/*     { */
	/*       P[offset + n].Mass = *fp++; */
      ...
      for(n = 0; n < pc; n++)
	{
	  if(RestartFlag == 2)
	    {
	      P[offset + n].DustRadius = (*fp++);
	    }
	  /* if (n%100 == 0){ */
	  /*   printf("particle %d, Dust Radius %g, %g, mass = %g, type = %d \n",offset+n,All.InitialDustRadius,P[offset+n].DustRadius,  */
	  /* 	   P[offset+n].Mass, P[offset + n].Type); */
	  /*   fflush(stdout); */
	}
#endif
      break;
    	  ...
    	  if(RestartFlag == 2)
    	    {
    	      ...
    	      P[offset + n].MicroDustMass = (*fp++);
    	      if (n%10000000 == 0)
    		{
    		  printf("particle %d, Micro dust %g, %g, mass = %g, type = %d \n",offset+n,P[offset+n].MicroDustMass, P[offset+n].DustRadius,
    			 P[offset+n].Mass, P[offset + n].Type);
    		  fflush(stdout);
    		}
#endif
    	      P[offset + n].DustVcoll = (*fp++);
#endif
    	    }
    	}
      break;
...
#ifdef VIRTUAL
      for(n = 0; n < pc; n++)
	P[offset + n].DeltaPhotonMomentum = *fp++;
#endif
      break;
...
#ifdef VIRTUAL
      for(n = 0; n < pc; n++)
	P[offset + n].OldPhotonMomentum = *fp++;
#endif
      break;
...
#ifdef VIRTUAL
      for(n = 0; n < pc; n++)
	P[offset + n].BirthPhotonEnergy = *fp++;
#endif
      break;
    ...
    case IO_DELPHOTONEN:
      ...
      for(n = 0; n < pc; n++)
	P[offset + n].DeltaPhotonEnergy = *fp++;
#endif
      break;
...
#ifdef VIRTUAL
      for(n = 0; n < pc; n++)
	SphP[offset + n].accreted_mass = *fp++;
#endif
#endif
      break;
    ...
    case IO_ACCMOM:
      ...
      for(n = 0; n < pc; n++)
	for(k = 0; k < 3; k++)
	SphP[offset + n].accreted_momentum[k] = *fp++;
#endif
#endif
      break;
    ...
    case IO_INJVIRTUALEN:
      ...
      for(n = 0; n < pc; n++)
	SphP[offset + n].Injected_VIRTUAL_Energy = *fp++;
#endif
      break;
...
#ifdef VIRTUAL_HEATING
      for(n = 0; n < pc; n++)
	SphP[offset + n].Emitted_VIRTUAL_Energy = *fp++;
#endif
      break;
...
#ifdef VIRTUAL_HEATING
      for(n = 0; n < pc; n++)
	SphP[offset + n].Returned_E_Fraction = *fp++;
#endif
      break;
