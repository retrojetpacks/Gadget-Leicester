...
static struct densdata_out
{
  ...
  MyLongDouble Micro_dust_Density;
#endif
      ...
      do
	{
	  ...
	  for(j = 0; j < nexport; j++)
	      ...
	      if(P[place].Type == 2)
		{
		  ...
		  P[place].d8.dDUST_micro_Density += DensDataOut[j].Micro_dust_Density;
#endif
...
#ifdef DUST_TWO_POPULATIONS
		P[i].d8.DUST_micro_Density = FLT(P[i].d8.dDUST_micro_Density);
#endif
...
#ifdef DUST_TWO_POPULATIONS
  MyLongDouble micro_dust_density = 0.;
#endif
  smoothentr = 0;
#endif
...
#ifdef DUST_TWO_POPULATIONS
		  micro_dust_density += FLT(P[j].MicroDustMass * wk);
#endif
#ifdef CONDUCTION_SATURATION
		  if(r > 0)
		    {
...
#ifdef DUST_TWO_POPULATIONS
      P[target].d8.dDUST_micro_Density =  micro_dust_density;
#endif
...
#ifdef DUST_TWO_POPULATIONS
      DensDataResult[target].Micro_dust_Density = micro_dust_density;
#endif
#endif
    }
