...
void greenf_diffusion(void)
{
  ...
  for(n = 0, NumSphUpdate = 0; n < N_gas; n++)
      ...
      if(P[n].Type == 0)
	{
	  ...
	  if(SphP[n].CR_E0 > 0)
	    {
	      CR_q_i = SphP[n].CR_q0 * pow(SphP[n].d.Density * a3inv, 0.33333);

	      cr_efac_i =
		CR_Tab_MeanEnergy(CR_q_i, All.CR_Alpha - 0.3333) / CR_Tab_MeanEnergy(CR_q_i, All.CR_Alpha);

	      kappa *= (All.CR_Alpha - 1) / (All.CR_Alpha - 1.33333) * pow(CR_q_i, 0.3333);
	  ...
	  SphP[n].CR_Kappa = kappa;
	  SphP[n].CR_Kappa_egy = kappa_egy;
	}
    }
		...
		if(Exportflag[j])
		  {
		    ...
		    DensDataIn[nexport].CR_Kappa = SphP[i].CR_Kappa;
		    DensDataIn[nexport].CR_Kappa_egy = SphP[i].CR_Kappa_egy;
		...
		if(Exportflag[j])
		  {
		    ...
		    DensDataIn[nexport].Hsml = PPP[i].Hsml;
		    DensDataIn[nexport].CR_E0 = SphP[i].CR_E0;
		    DensDataIn[nexport].CR_n0 = SphP[i].CR_n0;
  ...
  for(n = 0; n < N_gas; n++)
    {
      if(P[n].Type == 0)
	{
	  SphP[n].CR_E0 = SphP[n].CR_DeltaE;
	  SphP[n].CR_n0 = SphP[n].CR_DeltaN;
	  ...
	  if(SphP[n].CR_n0 > 1.0e-12 && SphP[n].CR_E0 > 0)
	    {
	      meanKineticEnergy = SphP[n].CR_E0 * m_p / SphP[n].CR_n0;

	      qmeanKin = CR_q_from_mean_kinetic_energy(meanKineticEnergy);

	      SphP[n].CR_q0 = qmeanKin * pow(SphP[n].d.Density * a3inv, -(1.0 / 3.0));
	      SphP[n].CR_C0 = SphP[n].CR_n0 * (All.CR_Alpha - 1.0) * pow(SphP[n].CR_q0, All.CR_Alpha - 1.0);
	    }
	  else
	    {
	      SphP[n].CR_E0 = 0.0;
