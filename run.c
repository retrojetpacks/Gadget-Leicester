...
/* SHC */
      All.NumLastDumpStep = All.NumCurrentTiStep;
/* SHC */
...
#ifdef DUMP_BY_NSTEP
if((All.NumCurrentTiStep != All.NumLastDumpStep) && (All.NumCurrentTiStep-All.NumLastDumpStep)%2000 == 0.) {
...
int ShouldWeDoDynamicUpdate(void)
{
  ...
  if(ThisTask == 0)
    printf("I'm guessing %d particles to be active in the next step\n", (int) numforces);
...
{
  double z,hubble_a;
  int i, tot, tot_sph, tot_count[TIMEBINS], tot_count_sph[TIMEBINS];
  ...
  if(ThisTask == 0)
	...
	printf("PM-Step. Total: %10d  %10d    Sum: %10d\n", tot - tot_sph, tot_sph, tot);
      else
#endif
	printf("Total active:   %10d  %10d    Sum: %10d\n", tot - tot_sph, tot_sph, tot);
...
/* SHC */
	      ...
	      All.CPU_Sum[CPU_BLACKHOLE], (All.CPU_Sum[CPU_BLACKHOLE]) / All.CPU_Sum[CPU_ALL] * 100,
/* SHC */
	      All.CPU_Sum[CPU_RHD], (All.CPU_Sum[CPU_RHD]) / All.CPU_Sum[CPU_ALL] * 100,
/* SHC */
...
void energy_statistics(void)
{
  ...
  if (All.NumCurrentTiStep == 0)
    All.total_energy_t0 = SysState.EnergyInt + SysState.EnergyPot + SysState.EnergyKin;
  double enrgy = SysState.EnergyInt + SysState.EnergyPot + SysState.EnergyKin;
  All.total_delta_energy = enrgy - All.total_energy_t0;
#endif
  ...
  if(ThisTask == 0)
    {
      fprintf(FdEnergy,
	      ...
	      SysState.MassComp[5]
             , SysState.Momentum[0], 
	      SysState.Momentum[1], SysState.Momentum[2], SysState.Momentum[3]);
  ...
  if(ThisTask == 0)
    {
      fprintf(FdEnergy,
	      ...
	      SysState.MassComp[1], SysState.MassComp[2], SysState.MassComp[3], SysState.MassComp[4],
	      SysState.MassComp[5]
             ,SysState.EnergyRadAdded,SysState.EnergyRadDeleted,SysState.EnergyRadComp);
